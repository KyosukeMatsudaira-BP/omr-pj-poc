...
def model_train(exp_name):
    ...
    config_dir = "config/"
    config_path = (
        config_dir + "model_architecture/omr_yolov5_change_anchor.yaml"
    )  #'yolov3/models/yolov5s.yaml'
    hyp_path = config_dir + "hyp/hyp.scratch-low.yaml"
    ...
    for epoch in range(num_epochs):
        ...
        output_path = fine_tune_dir + f"epoch{epoch + 1}/"
        os.makedirs(output_path, exist_ok=True)
        # トレーニング済みモデルの保存
        torch.save(model.state_dict(), output_path + "omr_yolov5s.pth")
